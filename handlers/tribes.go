...
func (th *tribeHandler) CreateOrEditTribe(w http.ResponseWriter, r *http.Request) {
	...
	if tribe.UUID == "" {
		utils.Log.Info("createOrEditTribe no uuid")
		w.WriteHeader(http.StatusUnauthorized)
		return
	}
	...
	if err != nil {
		utils.Log.Error("extract UUID error: %v", err)
		w.WriteHeader(http.StatusUnauthorized)
		return
	}
	...
	if pubKeyFromAuth == "" {
		...
		if pubKeyFromAuth != extractedPubkey {
			utils.Log.Info("createOrEditTribe pubkeys dont match")
			w.WriteHeader(http.StatusUnauthorized)
			return
		}
	}
	...
	if existing.UUID == "" { // if doesn't exist already, create unique name
		...
		if existing.OwnerPubKey != extractedPubkey {
			utils.Log.Info("createOrEditTribe tribe.ownerPubKey not match")
			utils.Log.Info("existing owner: %s", existing.OwnerPubKey)
			utils.Log.Info("extracted pubkey: %s", extractedPubkey)
			w.WriteHeader(http.StatusUnauthorized)
			return
		}
	}
	...
	if err != nil {
		utils.Log.Error("=> ERR createOrEditTribe: %v", err)
