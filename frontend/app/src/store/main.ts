...
export class MainStore {
    ...
    @action async getBots(
        ...
        b &&
            b.forEach((bb, i) => {
                ...
                if (
                    bb.unique_name === 'hello' ||
                    bb.unique_name === 'welcome'
                ) {
                ...
                if (
                    bb.unique_name &&
                    (bb.unique_name.includes('test') ||
                        hideBots.includes(bb.unique_name))
                ) {
                    // hide all test bots
                    bb.hide = true
                }
    ...
    @action async getMyBots(): Promise<any> {
        ...
        try {
            ...
            console.log('get bots')
            let tribeServerBots = await api.get(
                `bots/owner/${info.owner_pubkey}`
            )
    ...
    @action async getTorSaveURL(
        ...
        if (body.price_to_meet)
            body.price_to_meet = parseInt(body.price_to_meet)
    ...
    @action async saveProfile(body) {
        ...
        if (body.price_to_meet)
            body.price_to_meet = parseInt(body.price_to_meet) // must be an int
