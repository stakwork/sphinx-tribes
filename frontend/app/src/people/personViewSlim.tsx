...
export default function PersonView(props: any) {
    ...
    async function doDeeplink() {
        ...
        if (pathname) {
            ...
            if (personPubkey) {
                ...
                let widgetTimestamp: any = new URLSearchParams(search).get(
                    'timestamp'
                )
    ...
    function updatePathIndex(timestamp) {
        history.push(
            location.pathname +
                `?widget=${selectedWidget}&timestamp=${timestamp}`
        )
    }
    ...
    function renderWidgets(name: string) {
        if (name) {
            switch (name) {
                ...
                case 'post':
                    return wrapIt(
                        <PostView {...fullSelectedWidget} person={person} />
                    )
                case 'twitter':
                    return wrapIt(
                ...
                case 'offer':
                    return wrapIt(
                        <OfferView {...fullSelectedWidget} person={person} />
                    )
                case 'wanted':
                    return wrapIt(
                        <WantedView {...fullSelectedWidget} person={person} />
                    )
                case 'blog':
                    return wrapIt(
                        <BlogView {...fullSelectedWidget} person={person} />
                    )
                default:
                    return wrapIt(<></>)
            }
        ...
        const widgetSchema: any =
            (widgetSchemas &&
                widgetSchemas.find((f) => f.name === selectedWidget)) ||
            {}
